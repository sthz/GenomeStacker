...
def generateArrays(genbankfilecount,m8files):
  ...
  comparisonArrays = createm8Array(m8files)
  return("""
//global
var cache = [[]]
...
def createm8Array(m8files):
  ...
  for i in sys.argv:
    if i.endswith(".m8"):
      tempSubArray.append("'"+basename(i).replace(".m8",".json")+"'")
    else:
      if (len(tempSubArray) != 0):
        m8array.append("["+",".join(tempSubArray)+"]")
        tempSubArray = []
        comparisons.append("\"comparison_"+str(c+1)+"_"+str(c+2)+"\"")
        c += 1
  return("var jsonfiles = ["+",".join(m8array)+"]"+"\nvar comparisons = ["+",".join(comparisons)+"]"+"\n")
